...
export const Project = () => {
    ...
    const getProject = async (id) => {
        let project = await axios.get(`/projects/${id}`)
            .then(async res => {
                return res.data
            }).catch(err => {
                console.log(err)
        ...
        let owner = await axios.get(`/users/${project.userId}`)
            .then(res => {
                return res.data
            }
            ).catch(err => {
        ...
        if (project.userId === "f900d5e0-ab8d-410d-8792-95a64698bb6f") {
            setIsOwner(true)
        }
    }
    ...
    return isOwner ? <Owner project={project} getProject={getProject} owner={owner} /> : <NotOwner project={project} owner={owner} />
};

const Owner = ({ project, getProject, owner }) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    return <form className="w-full min-h-screen h-full p-0 flex flex-col items-center pb-16" onSubmit={handleSubmit(onSubmit)}>
        <div className="bg-[#061826] w-full px-8 md:px-16 py-8 md:py-16 flex flex-col md:flex-row justify-center items-center md:justify-between">
            ...
            <div className="bg-[#8A58DC] px-4 py-2 md:py-4 rounded-2xl shadow-lg md:absolute w-full md:w-1/4 flex flex-col justify-center md:right-0 mt-4 md:mr-16 md:top-0 md:mt-16">
                ...
                <div className="flex flex-col items-center">
                    <div className="mb-4 text-center">
                        <Title color={"#e2e2e2"} variant={3}>Owner</Title>
                        <p className="text-xl font-medium text-center">{owner.fullName ?? "Loading..."}</p>
                    </div>
                </div>
...
const NotOwner = ({ project, owner }) => {
    useEffect(() => {
        console.log("Not Owner")
    }, [])

    return (<div className="w-full min-h-screen h-full p-0 flex flex-col items-center pb-16">
        <div className="bg-[#061826] w-full px-8 md:px-16 py-8 md:py-16 flex flex-col md:flex-row justify-center items-center md:justify-between">
            <Link to={"/"} className="absolute text-white top-0 left-0 ml-4 md:ml-8 mt-4 md:mt-8 rotate-90">
                <Arrow color={"white"} width={36} />
            </Link>
            ...
            <div className="bg-[#8A58DC] px-4 py-4 md:py-8 rounded-2xl shadow-lg md:absolute w-full md:w-1/4 flex flex-col justify-center md:right-0 mt-4 md:mr-16 md:top-0 md:mt-16">
                ...
                <div className="flex flex-col items-center mt-4">
                    <Title color={"#e2e2e2"} variant={5}>Owner</Title>
                    <p className="text-lg font-medium text-center">{owner.fullName ?? "Loading..."}</p>
                </div>

                <div className="flex items-center">
                    <Calendar width={32} />
                    <p className="ml-2 font-medium text-2xl flex mt-4 mb-5 md:my-8">
                        {
                            Math.floor(
                                (new Date(project.endDate) - new Date(project.startDate)) / (1000 * 60 * 60 * 24)
                            )
